...
@section('content')
    <div class="row mr-3" dir="rtl">
        <h3 class="border-bottom border-2 border-white pb-1">نتایج جستجو برای: {{ $query }}</h3>
    </div>
    @if (sizeof($posts) > 0)
        @foreach ($posts as $post)
            <div class="card post-item bg-transparent border-0 mb-5">
                ...
                <div class="card-body px-0">
                    ...
                    <div class="d-flex flex-row p-2 align-items-center" style="gap:10px; text-align: right; direction: rtl">
                        @if (@$post->photos[0])
                            <img src=" {{ asset($post->photos[0]->path) }}"
                                style="min-width:128px;width:128px;height:128px;" class="rounded-circle" />
                        @else
                            <img src="{{ asset('img/default.jpg') }}" style="min-width:128px;width:128px;height:128px;"
            ...
            <!-- end of post-item -->
        @endforeach
        {{ $posts->appends(['title' => $query])->links() }}
    @else
        <h4 dir="rtl" class="mt-5 mr-3 text-primary" style="text-align: right">نتیجه ای برای عبارت {{$query}} یافت نشد!</h4>
    @endif
